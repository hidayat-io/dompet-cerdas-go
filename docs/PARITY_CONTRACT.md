...
* Kategori transaksi terselesaikan dengan confidence `high`: kecocokan langsung dengan nama kategori asli atau daftar alias kategori tanpa bantuan LLM, **atau** pilihan classifier ber-confidence `high` (ADR-018). Kategori ber-confidence `medium`/`low` maupun kategori fallback memblokir auto-save.
* Sumber pesan bukan voice note — voice tidak pernah auto-save dalam kondisi apa pun karena transkrip adalah lapisan tambahan tempat makna bisa meleset (ADR-018).

Jalur teks bebas (fallback Gemini LLM) tidak pernah mengisi `confidenceScore`, sehingga tetap selalu melewati draft konfirmasi. Jika salah satu kondisi tidak terpenuhi, backend wajib membuat session draf transaksi di Firestore dan membalas dengan tombol konfirmasi di Telegram. Backend Go harus menuliskan log audit terstruktur (`autoSaveTriggered`, nominal, deskripsi, kategori, serta `usedAI`, `confidenceScore` untuk jalur struk, dan `categoryViaClassifier` untuk asal kategori) saat auto-save berhasil dijalankan agar kesalahan sistem di produksi dapat dianalisis dengan mudah.
...
| Auto-save kategori teks | Kategori yang dipilih classifier LLM selalu mengirim pesan ke draft konfirmasi, walau nominal ter-parse deterministik oleh parser lokal — pesan seperti `2000 Beli PAT AI Qoder` (tanpa kata kunci kategori) selalu minta konfirmasi. | Kategori classifier ber-confidence `high` dipercaya: teks ber-item tunggal dengan nominal deterministik langsung auto-save; confidence `medium`/`low` dan fallback tetap konfirmasi; voice note tidak pernah auto-save (ADR-018). | Menghilangkan friksi konfirmasi pada pesan teks bernominal jelas; satu-satunya tebakan AI dibatasi ke kategori (dampak kosmetik, dapat diedit di web app) dan diaudit via `categoryViaClassifier`. | `internal/modules/telegram/draft_test.go` (`TestCategoryTrust_Record`, `TestShouldAutoSaveDraft_VoiceNeverAutoSaves`, `TestAutoSaveGate`) |
